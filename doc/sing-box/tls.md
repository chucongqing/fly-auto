---
title: "TLS - sing-box"
source: "https://sing-box.sagernet.org/zh/configuration/shared/tls/"
author:
  - "[[nekohasekai]]"
published:
created: 2026-06-04
...
为字符串时，共享 [证书提供者](https://sing-box.sagernet.org/zh/configuration/shared/certificate-provider/) 的标签。

为对象时，内联的证书提供者。可用类型和字段参阅 [证书提供者](https://sing-box.sagernet.org/zh/configuration/shared/certificate-provider/) 。
...
> 
> 如需 TLS 指纹抵抗，请改用 [NaiveProxy](https://sing-box.sagernet.org/zh/configuration/inbound/naive/) 。
...
在真实 ClientHello 之前注入一个伪造的、携带白名单 SNI 的 TLS ClientHello， 以欺骗基于 SNI 过滤的中间盒放行连接。

伪造报文是真实 ClientHello 的副本，仅将 SNI 值替换为本字段的值， 因此 TLS 指纹无法区分伪造与真实报文。真实服务器会丢弃伪造报文（见 `spoof_method` ）， 而中间盒将该连接视为合法会话。

需要原始套接字权限（Linux 上需 `CAP_NET_RAW` ，macOS 上需 root）； 在 Linux 上还需 `CAP_NET_ADMIN` ，因为需要通过 `TCP_REPAIR` 读取发送序列号。 Windows 上首次使用时需要 Administrator 以安装内嵌的 WinDivert 内核驱动， 不支持 Windows ARM64。
...
> 内联 ACME 选项已在 sing-box 1.14.0 废弃且将在 sing-box 1.16.0 中被移除，参阅 [迁移指南](https://sing-box.sagernet.org/zh/migration/#%E8%BF%81%E7%A7%BB%E5%86%85%E8%81%94-acme-%E5%88%B0%E8%AF%81%E4%B9%A6%E6%8F%90%E4%BE%9B%E8%80%85) 。
...
参阅 [DNS01 验证字段](https://sing-box.sagernet.org/zh/configuration/shared/dns01_challenge/) 。
...
握手服务器地址和 [拨号参数](https://sing-box.sagernet.org/zh/configuration/shared/dial/) 。
